...
public interface TagRepository extends JpaRepository<Tag, Long> {
  ...
  /**
   * Find a tag by exact name (global vocabulary lookup — used by tag-add for dedup). Tag names are
   * globally shared; only the per-image associations are owner-scoped.
   */
  Optional<Tag> findByName(String name);

  /** Find all tag names for an owner (for display/management). */
  @Query(
      "SELECT DISTINCT t.name FROM Tag t JOIN t.images i WHERE (:ownerId IS NULL AND i.ownerId IS NULL) OR i.ownerId = :ownerId ORDER BY t.name")
  ...
  @Query(
      "SELECT new com.imagegallery.dto.TagCountDto(t.name, COUNT(i)) FROM Tag t JOIN t.images i WHERE (:ownerId IS NULL AND i.ownerId IS NULL) OR i.ownerId = :ownerId GROUP BY t.id, t.name ORDER BY COUNT(i) DESC")
