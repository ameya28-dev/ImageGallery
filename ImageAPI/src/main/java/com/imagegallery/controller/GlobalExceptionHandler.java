...
public class GlobalExceptionHandler {
  ...
  /**
   * Handles authentication and authorization errors with detailed error types. Allows frontend to
   * distinguish between user-not-found, wrong password, etc.
   */
  ...
  /**
   * Handles API exceptions from external services (e.g., Anthropic Vision API). Returns the HTTP
   * status code indicated by the exception along with a user-friendly error message.
   */
  ...
  /**
   * Handles storage layer I/O exceptions (file not found, permission denied, etc.). Returns a clean
   * 500 error instead of letting the exception leak through filter chain and surface as an
   * inconsistent or cryptic status code.
   */
  ...
  /**
   * Catch-all fallback for any uncaught exception. Prevents exceptions from propagating to Spring's
   * error handler (/error dispatch), which would otherwise re-enter the security filter chain
   * without authentication and return a misleading 401 instead of the actual 500 error.
   */
