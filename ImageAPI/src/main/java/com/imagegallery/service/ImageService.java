...
public class ImageService {
    ...
    /**
     * Builds an owner-namespaced, collision-proof storage key.
     * Format: {ownerId|"public"}/{uuid}.{ext}
     */
    ...
    public List<ImageGroupDto> getImages(Long ownerId, String tag, List<String> tags, String type, Boolean favouritesOnly) {
        List<Image> images;
     ...
     * Only images owned by the caller are searched.
     */
    @Transactional(readOnly = true)
    public List<ImageGroupDto> searchByContent(Long ownerId, String q) {
        ...
        List<String> matchingTags = imageRepository.findTagNamesByPrefixForOwner(ownerId, pattern.replaceAll("%", ""));
    ...
    public List<Long> getImagesWithoutDescription(Long ownerId) {
        return imageRepository.findWithoutDescriptionForOwner(ownerId)
    ...
    /**
     * Fetch image by ID, verifying it belongs to the caller (or is in the public seed pool if caller is guest).
     * Deliberately throws not-found (404) on owner mismatch to avoid leaking other users' image IDs.
     */
    ...
    public StreamResult getThumbnailStream(Long ownerId, Long id) {
        ...
        String thumbKey = image.getThumbnailKey() != null ? image.getThumbnailKey() : image.getFilename();
        return new StreamResult(
    ...
    /**
     * Get thumbnail stream without ownership check — thumbnails are always public.
     * Used by the public endpoint that doesn't require authentication.
     */
    @Transactional(readOnly = true)
    ...
    /**
     * Upload and register an image for the caller.
     * Assigns ownerId, generates collision-proof storage keys, checks per-owner filename uniqueness.
     */
    @Transactional
    public ImageDto uploadAndRegister(Long ownerId, MultipartFile file, LocalDateTime takenAt) throws IOException {
        ...
        if (isVideo(filename)) {
            ...
            try {
                ...
                try (InputStream videoStream = new java.io.FileInputStream(tempVideo)) {
                    storageService.storeImage(imageKey, videoStream, tempVideo.length(), file.getContentType());
                }
            } finally {
                if (!tempVideo.delete()) {
            ...
            storageService.storeImage(imageKey, new ByteArrayInputStream(bytes), bytes.length, file.getContentType());
        }

        storageService.storeThumbnail(thumbKey, new ByteArrayInputStream(thumbBytes), thumbBytes.length);
    ...
    /**
     * Registers an image from the scanner (LocalImageScanner only).
     * Hardcodes ownerId = null (public seed pool) and seedSample = true.
     */
    @Transactional
    public Image registerScannedImage(String filename, String storageKey, String thumbnailKey, LocalDateTime takenAt) {
        Image image = new Image();
