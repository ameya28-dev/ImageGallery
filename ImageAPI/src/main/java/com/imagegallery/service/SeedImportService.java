...
/**
 * Handles importing seed-pool images into a user's private gallery on first login.
 * Performs server-side copy of both file bytes and metadata (tags, favourite status).
 */
@Slf4j
@Service
...
public class SeedImportService {
    ...
    /**
     * Import seed images into a user's private gallery.
     * Only callable when the user owns zero images (first-login guard).
     * Returns the list of newly-created image copies in the user's gallery.
     */
    public List<ImageDto> importSeedImages(Long ownerId) {
        ...
        List<Image> seeds = imageRepository.findByOwnerIdIsNullAndSeedSampleTrueOrderByTakenAtDescIdDesc();
        if (seeds.isEmpty()) {
        ...
        for (Image seed : seeds) {
            try {
                ...
                String sourceThumbKey = seed.getThumbnailKey() != null ? seed.getThumbnailKey() : seed.getFilename();
                ...
                log.info("Imported seed image {} ({} -> {}) into user {}'s gallery", seed.getFilename(), sourceImageKey, newImageKey, ownerId);
                // Note: AI description already copied from seed (copy.setAiDescription(seed.getAiDescription())),
                // so no need to call describeAndSave() — it would be a no-op due to the getAiDescription() != null check.
            } catch (Exception e) {
