...
/** Service for video processing using FFmpeg. Handles frame extraction and metadata reading. */
@Slf4j
@Service
@RequiredArgsConstructor
public class VideoService {

  /** Extract the first frame from a video file as a JPEG. Returns the JPEG image bytes. */
    ...
    try {
      ...
      if (process.exitValue() != 0) {
        throw new IOException(
            "FFmpeg frame extraction failed with exit code: " + process.exitValue());
      }
  ...
  /** Get the duration of a video file in seconds. Returns -1.0 if duration cannot be determined. */
  ...
  /**
   * Format duration in seconds to HH:MM:SS or MM:SS format. Uses HH:MM:SS if duration >= 1 hour,
   * otherwise MM:SS.
   */
