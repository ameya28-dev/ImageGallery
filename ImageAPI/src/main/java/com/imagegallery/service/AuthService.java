  ...
  public AuthResponse login(LoginRequest request, HttpServletResponse response) {
    ...
    if (userOpt.isEmpty()) {
      ...
      throw new AuthException(
          "Account doesn't exist. Please create one.", AuthException.ErrorType.USER_NOT_FOUND, 400);
    }
    ...
    try {
      ...
      // Wrong password
      throw new AuthException(
          "Incorrect password. Please try again.", AuthException.ErrorType.INVALID_PASSWORD, 401);
    }
  ...
  public AuthResponse refresh(String oldRefreshToken, HttpServletResponse response) {
    ...
    User user =
        userRepository
            ...
            .filter(
                u ->
                    u.getRefreshTokenExpiry() != null
                        && u.getRefreshTokenExpiry().isAfter(LocalDateTime.now()))
            .orElseThrow(
                () ->
                    new ResponseStatusException(HttpStatus.UNAUTHORIZED, "Refresh token mismatch"));
    return issueTokenPair(user, response);
  }
