...
function SelectionHeader({
  ...
  return (
    <div className="sticky top-0 z-30 flex items-center justify-between border-b border-gray-800 bg-black px-4 py-4">
      <button
        onClick={onSelectAll}
...
export default function GalleryPage() {
  ...
  const [showBulkDeleteLoginPrompt, setShowBulkDeleteLoginPrompt] =
    useState(false);
  ...
  useEffect(() => {
    const seedImportEnabled =
      process.env.NEXT_PUBLIC_SEED_IMPORT_ENABLED === "true";
  ...
  const handleBulkDelete = useCallback(async () => {
    ...
    setGroups((prev) =>
      prev
        .map((g) => ({
          ...g,
          images: g.images.filter((img) => !deletedSet.has(img.id)),
        }))
        ...
        for (let i = 0; i < files.length; i++) {
          try {
            ...
            if (errorMsg.includes("already exists")) {
              console.warn(
                `Duplicate detected: ${files[i].name} - ${errorMsg}`,
              );
            } else {
  ...
  if (error) {
    return (
      <div className="flex min-h-screen flex-col items-center justify-center gap-4">
        ...
        <button onClick={reload} className="text-sm text-blue-400">
          Retry
        </button>
      </div>
    );
  ...
  return (
    <main className="pb-20">
      {selectionMode ? (
        ...
        <GalleryHeader
          onSelectMode={() => {
            if (isOwner) setSelectionMode(true);
          }}
      ...
      {filteredGroups.length === 0 && (
        <div className="flex flex-col items-center justify-center gap-6 px-8 py-24">
          <div className="flex h-52 w-52 items-center justify-center rounded-3xl bg-neutral-950 p-5">
            <img
              src="/noImages.svg"
              alt=""
              aria-hidden="true"
      ...
      <TagDialog
        image={
          bulkTagOpen && selectedIds.size > 0
            ? bulkTagTarget(selectedIds.size)
            : null
        }
        open={bulkTagOpen}
        onClose={() => {
      ...
      {dragOver && (
        <div className="pointer-events-none fixed inset-0 z-[70] flex items-center justify-center bg-black/80">
          <div className="flex flex-col items-center gap-3 rounded-3xl border-2 border-dashed border-blue-400 px-16 py-14 text-center">
            <p className="text-2xl font-bold text-white">Drop to upload</p>
            <p className="text-sm text-gray-400">
              Release to add images to your gallery
            </p>
          </div>
        </div>
      )}
